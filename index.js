...
if (navigator.getBattery()) {
  ...
  navigator
    ...
    .then((dat) => {
      ...
      isCharging
        ? powerLevel.forEach((a) => {
            a.textContent = `Device is charging`;
          })
        : powerLevel.forEach((a) => {
            a.textContent = `Device is not charging`;
          });

      // View battery Level
      chargeLevel == 100
        ? chargerState.forEach((a) => {
            a.textContent = `Your device is fully charged`;
          })
        : chargerState.forEach((a) => {
            a.textContent = `Your device is currently ${chargeLevel}`;
          });
    });
} else {
